...
var getCategoryIdsForExclusion = function(array, categories) {
  if (categories != null) {
    categories.forEach(function(c) {
      array.push(c._id);
      var cats = Pos.Collection.Categories.find({
        parentId: c._id
      });
      if (cats != null) {
...
Pos.List = {
  tableLocations: function() {
    ...
    Pos.Collection.TableLocations.find({
  ...
  category: function(param) {
    ...
    var alreadyUse = [];
    Pos.Collection.Categories.find(selector, {
      sort: {
        level: 1
      }
    }).forEach(function(obj) {
  ...
  subCategory: function() {
    ...
    if (categoryId == null) {
      ...
      Pos.Collection.SubCategories.find({

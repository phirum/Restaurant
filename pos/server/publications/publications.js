...
Meteor.publish('posTableLocation', function (selector) {
    if (this.userId) {
        selector = selector == null ? {} : selector;
        return Pos.Collection.TableLocations.find(selector, {removed: true});
    }
});

Meteor.publish('posTable', function (selector) {
    if (this.userId) {
        selector = selector == null ? {} : selector;
        return Pos.Collection.Tables.find(selector, {removed: true});
    }
});

/*function fifoInventoryInsert(branchId, pd, prefix) {
 ...
 if (inventory == null) {
 var inventoryObj = {};
 inventoryObj._id = idGenerator.genWithPrefix(Restaurant.Collection.FIFOInventory, prefix, 13);
 ...
 Restaurant.Collection.FIFOInventory.insert(inventoryObj);
 }
 ...
 Restaurant.Collection.FIFOInventory.update(inventory._id, {$set: inventorySet});

 }
 else {
 var nextInventory = {};
 nextInventory._id = idGenerator.genWithPrefix(Restaurant.Collection.FIFOInventory, prefix, 13);
 ...
 Restaurant.Collection.FIFOInventory.insert(nextInventory);
 }
 }*/
function fifoInventoryInsert(branchId, pd, prefix) {
//FIFO find the last record that has the same price
...
//LIFO find the last record that has the same price
    ...
    var lastRecord = Restaurant.Collection.FIFOInventory.findOne({
        branchId: branchId,
        productId: pd.productId
        //price: pd.price
    }, {sort: {createdAt: 1}});
...
Meteor.methods({
    insertStock: function (obj) {
        Restaurant.Collection.Stocks.insert(obj);
    },
    directInsertStock: function (obj) {
        Restaurant.Collection.Stocks.direct.insert(obj);
    },
    updateStock: function (id, set, unset) {
        var updateObject = {};
        if (set != null) updateObject.$set = set;
        if (unset != null) updateObject.$unset = unset;
        Restaurant.Collection.Stocks.update(id, updateObject);
    },
    directUpdateStock: function (id, set, unset) {
        var updateObject = {};
        if (set != null) updateObject.$set = set;
        if (unset != null) updateObject.$unset = unset;
        Restaurant.Collection.Stocks.direct.update(id, updateObject);
    },
    insertStockHistory: function (branchId) {
        var products = Restaurant.Collection.Products.find({productType: "Stock"});
        //var branches = Cpanel.Collection.Branch.find();
        var today = moment(new Date).format('YYYYMMDD');
        var prefix = branchId + "-" + today + "-";
        var obj = {};
        obj._id = idGenerator.genWithPrefix(Restaurant.Collection.StockHistories, prefix, 3);
        obj.stockList = [];
        // branches.forEach(function (b) {
        products.forEach(function (p) {
            var stock = Restaurant.Collection.Stocks.findOne({productId: p._id, branchId: branchId});
        ...
        return Restaurant.Collection.StockHistories.insert(obj);
        // });
    ...
    purchaseManageStock: function (purchaseId, branchId) {
        Meteor.defer(function () {
            ...
            purchaseDetails.forEach(function (pd) {
                ...
                Restaurant.Collection.Products.direct.update(pd.productId, {$set: productSet});
    ...
    saleManageStock: function (saleId, branchId) {
        Meteor.defer(function () {
            ...
            saleDetails.forEach(function (sd) {
                    var transaction = [];
                    var inventories = Restaurant.Collection.FIFOInventory.find({
                        branchId: branchId,
                    ...
                    for (var i = 0; i < inventories.length; i++) {
                        ...
                        Restaurant.Collection.FIFOInventory.update(inventories[i]._id, {$set: inventorySet});
    ...
    locationTransferManageStock: function (locationTransferId, branchId) {
        Meteor.defer(function () {
            ...
            var locationTransferDetails = Restaurant.Collection.LocationTransferDetails.find({locationTransferId: locationTransferId});
            var prefix = branchId + "-";
            locationTransferDetails.forEach(function (ltd) {
                    ...
                    var setObj = {};
                    setObj.transaction = transaction;
                    Restaurant.Collection.LocationTransferDetails.direct.update(
                        ltd._id,
                        {$set: setObj}
                    );
                    //inventories=sortArrayByKey()

Restaurant.Collection.Payments.before.insert(function (userId, doc) {
    var prefix;
    prefix = doc.saleId ? doc.saleId + "S" : doc.purchaseId + "P";
    doc._id = idGenerator.genWithPrefix(Restaurant.Collection.Payments, prefix, 3);
});

Restaurant.Collection.Payments.after.remove(function (userId, doc) {
    Meteor.defer(function () {
        if (doc.saleId) {
            var saleObj = {};
            saleObj.owedAmount = doc.dueAmount;
            saleObj.status = "Owed";
            Restaurant.Collection.Sales.direct.update(doc.saleId, {$set: saleObj});
        } else {
            var purchaseObj = {};
            purchaseObj.owedAmount = doc.dueAmount;
            purchaseObj.status = "Owed";
            Restaurant.Collection.Purchases.direct.update(doc.purchaseId, {$set: purchaseObj});
        }
    });

});

Restaurant.Collection.Payments.after.insert(function (userId, doc) {
    Meteor.defer(function () {
        if (doc.saleId) {
            var saleObj = {};
...
Restaurant.Collection.Payments.after.update(function (userId, doc, fieldNames, modifier, options) {
    Meteor.defer(function () {
        if (doc.saleId) {
            var saleObj = {};
            if (doc.balanceAmount <= 0) {

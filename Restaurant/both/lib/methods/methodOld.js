...
Meteor.methods({
    getProductSaleReport: function () {
            var sD = Restaurant.Collection.SaleDetails.aggregate([{
            $group: {
        ...
        sD.forEach(function (s) {
            var product = Restaurant.Collection.findOne(s._id.productId);
    ...
    insertSaleAndSaleDetail: function (sale, saleDetail) {
        ...
        var saleId = Restaurant.Collection.Sales.insert(sale);

        saleDetail._id = idGenerator.genWithPrefix(Restaurant.Collection.SaleDetails, saleId, 3);
    ...
    insertAdjustmentAndAdjustmentDetail: function (adjustment, adjustmentDetail) {
        ...
        adjustmentDetail._id = idGenerator.genWithPrefix(Restaurant.Collection.AdjustmentDetails, adjustmentId, 3);
    ...
    insertAdjustment: function (obj) {
        ...
        obj.createdUserId = Meteor.user()._id;
        obj.updatedUserId = Meteor.user()._id;
        return Restaurant.Collection.Adjustments.insert(obj);
    },
    updateAdjustment: function (id, set) {
        set.updatedAt = new Date();
        set.updatedUserId = Meteor.user()._id;
        Restaurant.Collection.Adjustments.update(id, {$set: set});
    },
    ...
    updateAdjustmentDetails: function (id, set) {
        set.updatedAt = new Date();
        set.updatedUserId = Meteor.user()._id;
        Restaurant.Collection.AdjustmentDetails.update(id, {$set: set});
    },
    ...
    updateStock: function (id, set) {
        set.updatedAt = new Date();
        set.updatedUserId = Meteor.user()._id;
        Restaurant.Collection.Stocks.update(id, {$set: set});
    },

...
Template.restaurant_adjustment.onRendered(function () {
    ...
    setTimeout(function () {
        $('.select-two').select2();
        var s = Restaurant.Collection.Adjustments.findOne({
            _id: FlowRouter.getParam('adjustmentId'),
            status: "Unsaved",
            branchId: Session.get('currentBranch')
        });
        if (s == null) {
            FlowRouter.go('restaurant.adjustment');
            $('#product-barcode').focus();
        }
    }, 700);
});

Template.restaurant_adjustment.helpers({
    ...
    adjustmentDate: function () {
        var adjustment = Restaurant.Collection.Adjustments.findOne(FlowRouter.getParam('adjustmentId'));
        if (adjustment == null) {
            return "";
    ...
    adjustment: function () {
        var s = Restaurant.Collection.Adjustments.findOne(FlowRouter.getParam('adjustmentId'));
    ...
    adjustmentDetails: function () {
        ...
        var sD = Restaurant.Collection.AdjustmentDetails.find({adjustmentId: FlowRouter.getParam('adjustmentId')});
        var i = 1;
        sD.forEach(function (sd) {
            // var item = _.extend(sd,{});
           /* var product = Restaurant.Collection.Products.findOne(sd.productId);
            sd.unit = Restaurant.Collection.Units.findOne(product.unitId).name;
    ...
    adjustments: function () {
        ...
        if (id != null || id != "") {
            return Restaurant.Collection.Adjustments.find({
                _id: {$ne: id},
                branchId: Session.get('currentBranch'),
                status: "Unsaved"
            });
...
Template.restaurant_adjustment.events({
    ...
    'change .new-qty, mouseleave .new-qty': function (e) {
        ...
        // var numericReg = /^\d*[0-9](|.\d*[0-9]|,\d*[0-9])?$/; //restaurantitive number
        //var numericReg = /^[+-]?[0-9]{1,9}(?:\.[0-9]{1,2})?$/; //negative and restaurantitive decimal number
        var numericReg = /^[+-]?[0-9]{1,9}(?:\[0-9]{1,2})?$/; //negative and restaurantitive number
        ...
        var price = Restaurant.Collection.Products.findOne(this.productId).purchasePrice;
    ...
    'click .btn-remove': function () {
        Restaurant.Collection.AdjustmentDetails.remove(this._id);
        var sd = Restaurant.Collection.AdjustmentDetails.find({adjustmentId: FlowRouter.getParam('adjustmentId')});
        if (sd.count() == 0) {
            Restaurant.Collection.Adjustments.remove(FlowRouter.getParam('adjustmentId'));
            FlowRouter.go('restaurant.adjustment');
            prepareAdjustmentForm();
        } else {
            updateAdjustmentTotalDiffCost(FlowRouter.getParam('adjustmentId'));
    ...
    'click .staffInsertAddon': function () {
        alertify.userStaff(renderTemplate(Template.restaurant_userStaffInsert))
            .set({
    ...
    'click .supplierInsertAddon': function () {
        alertify.supplier(renderTemplate(Template.restaurant_supplierInsert))
            .set({
    ...
    'change #product-id': function () {
        ...
        var product = Restaurant.Collection.Products.findOne(id);
        if (product != null) {
            var adjustmentId = $('#adjustment-id').val();
    ...
    'keyup #product-barcode': function (e) {
        ...
        if (e.which == 13) {
            var product = Restaurant.Collection.Products.findOne({barcode: $('#product-barcode').val(), status: "enable"});
            if (product != null) {
                var adjustmentId = $('#adjustment-id').val();
                addOrUpdateProductsToList(adjustmentId, product);
            } else {
...
function addOrUpdateProductsToList(adjustmentId, product) {
    var stock = Restaurant.Collection.Stocks.findOne({productId: product._id, branchId: Session.get('currentBranch')});
    ...
    if (adjustmentId == '') {
        ...
        var newId = idGenerator.genWithPrefix(Restaurant.Collection.Adjustments, prefix, 4);
        ...
        FlowRouter.go('restaurant.adjustment', {adjustmentId: newId});
        $('#product-barcode').focus();
        //
    } else {
        var adjustmentDetail = Restaurant.Collection.AdjustmentDetails.findOne({
            productId: product._id,
            adjustmentId: adjustmentId
        });
        if (adjustmentDetail == null) {
            var adjustmentDetailObj = {};
            adjustmentDetailObj._id = idGenerator.genWithPrefix(Restaurant.Collection.AdjustmentDetails, adjustmentId, 3);
...
updateAdjustmentTotalDiffCost = function (adjustmentId) {
    ...
    var oldQty = 0;
    var newQty = 0;
    var adjustmentDetails = Restaurant.Collection.AdjustmentDetails.find({adjustmentId: adjustmentId});
...
function payAdjustment(adjustmentId) {
    ...
    adjustmentDetails.forEach(function (ad) {
        var product = Restaurant.Collection.Products.findOne(ad.productId);
        if (product.productType == "Stock") {
            var stock = Restaurant.Collection.Stocks.findOne({productId: ad.productId, branchId: branchId});
            if (stock == null) {
                var obj = {};
...
function checkAdjustmentIsUpdate() {
    ...
    var adjustment = Restaurant.Collection.Adjustments.findOne(adjustmentId);
    var reason = $('#reason').val();
    var staff = $('#staff-id').val();
    var date = $('#input-adjustment-date').val();
    var description = $('#description').val();

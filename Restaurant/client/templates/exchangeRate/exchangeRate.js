var restaurantExchangeRateTPL=Template.restaurant_exchangeRate;
var restaurantExchangeRateInsertTPL=Template.restaurant_exchangeRateInsert;
restaurantExchangeRateTPL.onRendered(function () {
    createNewAlertify("exchangeRate");
});
restaurantExchangeRateTPL.events({
    'click #add-exchange-rate': function (e, t) {
        alertify.exchangeRate(fa('plus','Add New Exchange Rate'),renderTemplate(restaurantExchangeRateInsertTPL));
    }
});
restaurantExchangeRateTPL.helpers({
    selector: function () {
    ...
    currentExchangeRate: function () {
        ...
        return Restaurant.Collection.ExchangeRates.findOne({base: id,branchId:Session.get('currentBranch')}, {sort: {_id: -1}});
    },
    exchangeRates: function () {
        var id = Cpanel.Collection.Setting.findOne().baseCurrency;
        // return Restaurant.Collection.ExchangeRates.findOne({base:id},{sort:{_id:-1,createdAt:-1}});
        return Restaurant.Collection.ExchangeRates.find({branchId:Session.get('currentBranch')});
    },
...
restaurantExchangeRateInsertTPL.events({
...
restaurantExchangeRateInsertTPL.helpers({
    /*currentExchangeRate:function(){
     var id = Cpanel.Collection.Setting.findOne().baseCurrency;
     return Restaurant.Collection.ExchangeRates.findOne({base:id},{sort:{_id:-1,createdAt:-1}});
     },*/

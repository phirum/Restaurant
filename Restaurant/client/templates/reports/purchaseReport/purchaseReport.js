...
Template.restaurant_purchaseReport.onRendered(function () {
    var name = $('[name="date"]');
    DateTimePicker.dateRange(name);
});
Template.restaurant_purchaseReport.events({
...
Template.restaurant_purchaseReportGen.helpers({
    ...
    data: function () {
        ...
        var call = Meteor.callAsync(callId, 'restaurantPurchaseReport', q);
        if (!call.ready()) {
            return false;
        }
        return call.result();
...
Template.restaurant_purchaseReportGen.helpers({
    ...
    reportHelper: function () {
        ...
        params.status = "Paid";
        var purchase = Restaurant.Collection.Purchases.find(params);
        ...
        if (supplierId != null && supplierId != "")
            supplier = Restaurant.Collection.Suppliers.findOne(supplierId).name;
...
function calculatePurchaseHelper(pur) {
    ...
    pur.forEach(function (p) {
        ...
        p.supplier = Restaurant.Collection.Suppliers.findOne(p.supplierId).name;
        p.staff = Restaurant.Collection.Staffs.findOne(p.staffId).name;
        p.order = i;
        p.exchangeRates = [];
        Restaurant.Collection.ExchangeRates.findOne(p.exchangeRateId).rates.
            forEach(function (ex) {

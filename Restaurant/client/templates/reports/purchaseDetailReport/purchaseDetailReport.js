...
Template.restaurant_purchaseDetailReport.onRendered(function () {
    var name = $('[name="date"]');
    DateTimePicker.dateRange(name);
});
Template.restaurant_purchaseDetailReport.events({
...
Template.restaurant_purchaseDetailReportGen.helpers({
    options: function () {
    ...
    data: function () {
        ...
        var call = Meteor.callAsync(callId, 'restaurantPurchaseDetailReport', q);
        if (!call.ready()) {
            return false;
        }
        return call.result();
...
function getPurchaseProducts(params) {
    var purchaseIds = Restaurant.Collection.Purchases.find(params, {fields: {_id: 1}}).map(function (purchase) {
        return purchase._id;
    });
    var result = [];
    var purchaseDetails = Restaurant.Collection.PurchaseDetails.find(

var restaurantPPTPL = Template.restaurant_promotionPercentage;
var restaurantPPInsertTPL = Template.restaurant_promotionPercentageInsert;
var restaurantPPUpdateTPL = Template.restaurant_promotionPercentageUpdate;
var restaurantPPShowTPL = Template.restaurant_promotionPercentageShow;

restaurantPPTPL.onRendered(function () {
    createNewAlertify(['promotionPercentage', 'promotionPercentageShow']);
});
restaurantPPTPL.events({
    'click .insert': function (e, t) {
        alertify.promotionPercentage(fa('plus', 'Add New PromotionPercentage'), renderTemplate(restaurantPPInsertTPL)).maximize();
    },
    'click .update': function (e, t) {
        var data = Restaurant.Collection.PromotionPercentages.findOne(this._id);
        alertify.promotionPercentage(fa('pencil', 'Update Existing PromotionPercentage'), renderTemplate(restaurantPPUpdateTPL, data)).maximize();
            ...
            .set({
                onok: function (closeEvent) {
                    var relation = relationExist(
                        [
                            {collection: Restaurant.Collection.Sales, selector: {promotionPercentageId: id}}
                        ]
                    );
                    if (relation) {
                        alertify.alert("Data has been used. Can't remove.").set({title: "Data has been used."});
                    } else {
                        Restaurant.Collection.PromotionPercentages.remove(id, function (error) {
                            if (error) {
    ...
    'click .show': function (e, t) {
        ...
        alertify.promotionPercentageShow(fa('eye', 'Promotion Detail'), renderTemplate(restaurantPPShowTPL, this));
    }
});
restaurantPPInsertTPL.onRendered(function () {
    /*
...
restaurantPPUpdateTPL.onRendered(function () {
    /*this.$('.startDate').datetimepicker({
     format: 'YYYY-MM-DD HH:mm:ss'
     });
     this.$('.endDate').datetimepicker({
...
restaurantPPUpdateTPL.helpers({
    formatDate: function (date) {
        return moment(date).format('YYYY-MM-DD HH:mm:ss');
    }
});
AutoForm.hooks({
    restaurant_promotionPercentageInsert: {
        before: {
            insert: function (doc) {
                debugger;
                ...
                if (startDate != '' && endDate != '') {
                    ...
                    var promotion = Restaurant.Collection.PromotionPercentages.find(selector);
    ...
    restaurant_promotionPercentageUpdate: {
        before: {
            update: function (doc) {

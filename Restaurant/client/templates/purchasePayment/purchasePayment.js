var restaurantPurchasePaymentTPL = Template.restaurant_purchasePayment;
var restaurantPurchasePaymentInsertTPL = Template.restaurant_purchasePaymentInsert;
var restaurantPurchasePaymentUpdateTPL = Template.restaurant_purchasePaymentUpdate;
var restaurantPurchasePaymentShowTPL = Template.restaurant_purchasePaymentShow;

restaurantPurchasePaymentTPL.onRendered(function () {
    createNewAlertify(['purchasePayment', 'purchasePaymentShow']);
});
restaurantPurchasePaymentTPL.helpers({
    selector: function () {
        var selector = {};
...
restaurantPurchasePaymentTPL.events({
    'click .insert': function (e, t) {
        alertify.purchasePayment(fa('plus', 'Add New Payment'), renderTemplate(restaurantPurchasePaymentInsertTPL)).maximize();
    },
    'click .update': function (e, t) {
        ...
        if (data._id == payment._id && data.status != "firstPay") {
            alertify.purchasePayment(fa('pencil', 'Update Existing Payment'), renderTemplate(restaurantPurchasePaymentUpdateTPL, data)).maximize()
        } else {
    ...
    'click .show': function (e, t) {
        alertify.purchasePaymentShow(fa('eye', 'Payment Detail'), renderTemplate(restaurantPurchasePaymentShowTPL, this));
    }
});
restaurantPurchasePaymentInsertTPL.onRendered(function () {
    datePicker();
});
restaurantPurchasePaymentInsertTPL.helpers({
    dueAmount: function () {
    ...
    exchangeRates: function () {
        var id = Cpanel.Collection.Setting.findOne().baseCurrency;
        return Restaurant.Collection.ExchangeRates.findOne({base: id, branchId: Session.get('currentBranch')}, {
            sort: {
                _id: -1,
                createdAt: -1
            }
...
restaurantPurchasePaymentInsertTPL.events({
...
restaurantPurchasePaymentUpdateTPL.helpers({
...
restaurantPurchasePaymentUpdateTPL.onRendered(function () {
    datePicker();
});

restaurantPurchasePaymentUpdateTPL.events({
...
AutoForm.hooks({
    restaurant_purchasePaymentInsert: {
    ...
    restaurant_purchasePaymentUpdate: {
        onSuccess: function (formType, result) {

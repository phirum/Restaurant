var restaurantPATPL = Template.restaurant_promotionTotalAmount;
var restaurantPAInsertTPL = Template.restaurant_promotionTotalAmountInsert;
var restaurantPAUpdateTPL = Template.restaurant_promotionTotalAmountUpdate;
var restaurantPAShowTPL = Template.restaurant_promotionTotalAmountShow;

restaurantPATPL.onRendered(function () {
    createNewAlertify(['promotionTotalAmount', 'promotionTotalAmountShow']);
});
restaurantPATPL.events({
    'click .insert': function (e, t) {
        alertify.promotionTotalAmount(
            fa('plus', 'Add New Promotion By Amount'),
            renderTemplate(restaurantPAInsertTPL)
        ).maximize();
    },
    'click .update': function (e, t) {
        var data = Restaurant.Collection.PromotionTotalAmounts.findOne(this._id);
        alertify.promotionTotalAmount(
            fa('pencil', 'Update Existing Promotion By Amount'),
            renderTemplate(restaurantPAUpdateTPL, data)
        ).maximize();
    },
            ...
            .set({
                onok: function (closeEvent) {
                    var relation = relationExist(
                        [
                            {collection: Restaurant.Collection.Sales, selector: {promotionTotalAmountId: id}}
                        ]
                    );
                    if (relation) {
                        alertify.alert("Data has been used. Can't remove.").set({title: "Data has been used."});
                    } else {
                        Restaurant.Collection.PromotionTotalAmounts.remove(id, function (error) {
                            if (error) {
    ...
    'click .show': function (e, t) {
        ...
        alertify.promotionTotalAmountShow(
            ...
            renderTemplate(restaurantPAShowTPL, this)
        );

    }
});
restaurantPAInsertTPL.onRendered(function () {
...
restaurantPAUpdateTPL.onRendered(function () {
    /*this.$('.startDate').datetimepicker({
     format: 'YYYY-MM-DD HH:mm:ss'
     });
     this.$('.endDate').datetimepicker({
...
restaurantPAUpdateTPL.helpers({
    formatDate: function (date) {
        return moment(date).format('YYYY-MM-DD HH:mm:ss');
    }
});
AutoForm.hooks({
    restaurant_promotionTotalAmountInsert: {
        before: {
            insert: function (doc) {
                ...
                if ($('[name="startDate"]').val() != '' && $('[name="endDate"]').val() != '') {
                    ...
                    var promotion = Restaurant.Collection.PromotionTotalAmounts.find(selector);
    ...
    restaurant_promotionTotalAmountUpdate: {
        before: {
            update: function (doc) {
                if ($('[name="startDate"]').val() != '' && $('[name="endDate"]').val() != '') {

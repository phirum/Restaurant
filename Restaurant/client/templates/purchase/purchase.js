...
Template.restaurant_purchase.onRendered(function () {
    ...
    setTimeout(function () {
        $('.select-two').select2();
        var s = Restaurant.Collection.Purchases.findOne({
...
Template.restaurant_purchase.helpers({
    locations: function () {
    ...
    imeis: function () {
        ...
        if (purchaseDetailId != null) {
            var pd = Restaurant.Collection.PurchaseDetails.findOne(purchaseDetailId);
    ...
    purchase: function () {
        var p = Restaurant.Collection.Purchases.findOne(FlowRouter.getParam('purchaseId'));
    ...
    purchaseDetails: function () {
        ...
        var sD = Restaurant.Collection.PurchaseDetails.find({purchaseId: FlowRouter.getParam('purchaseId')});
        var i = 1;
    ...
    purchases: function () {
        ...
        if (id != null || id != "") {
            return Restaurant.Collection.Purchases.find({
                _id: {$ne: id},
                branchId: Session.get('currentBranch'),
                status: "Unsaved"
            });
...
Template.restaurant_purchase.events({
    ...
    'click .btn-remove-imei': function (e) {
        ...
        var imei = this;
        var purchaseDetail = Restaurant.Collection.PurchaseDetails.findOne(purchaseDetailId);
        var obj = {};
    ...
    'click .resume': function (e) {
        ...
        var purchase = Restaurant.Collection.Purchases.findOne(purchaseId);
    ...
    'change #total_discount': function (e) {
        ...
        /*   Restaurant.Collection.Purchases.update($('#purchase-id').val(),
         {
         $set: {
         discount: discount,
         total: total,
    ...
    'change .price': function (e) {
        ...
        var product = Restaurant.Collection.Products.findOne(this.productId);
    ...
    'click .btn-remove': function () {
        Restaurant.Collection.PurchaseDetails.remove(this._id);
        var sd = Restaurant.Collection.PurchaseDetails.find({purchaseId: FlowRouter.getParam('purchaseId')});
        if (sd.count() == 0) {
            Restaurant.Collection.Purchases.remove(FlowRouter.getParam('purchaseId'));
            FlowRouter.go('restaurant.purchase');
            preparePurchaseForm();
        }
        /*else {
    ...
    'click .supplierInsertAddon': function () {
        alertify.supplier(fa('plus', 'Add New Supplier'), renderTemplate(Template.restaurant_supplierInsert));
        // .maximize();
    },
...
function addOrUpdateProducts(branchId, purchaseId, product, purchaseObj) {
    ...
    if (purchaseId == '') {
        ...
        var newId = idGenerator.genWithPrefix(Restaurant.Collection.Purchases, prefix, 4);
        ...
        FlowRouter.go('restaurant.purchase', {purchaseId: newId});
        $('#product-barcode').focus();
        //
    } else {
        var purchaseDetail = Restaurant.Collection.PurchaseDetails.findOne({
        ...
        if (purchaseDetail == null) {
            var purchaseDetailObj = {};
            purchaseDetailObj._id = idGenerator.genWithPrefix(Restaurant.Collection.PurchaseDetails, purchaseId, 3);
...
function pay(purchaseId) {
    ...
    var baseCurrencyId = Cpanel.Collection.Setting.findOne().baseCurrency;
    obj._id = idGenerator.genWithPrefix(Restaurant.Collection.Payments, purchaseId, 3);

...
Template.restaurant_locationTransfer.onRendered(function () {
    ...
    setTimeout(function () {
        $('.select-two').select2();
        var s = Restaurant.Collection.LocationTransfers.findOne({
            _id: FlowRouter.getParam('locationTransferId'),
            status: "Unsaved",
            branchId: Session.get('currentBranch')
        });
        if (s == null) {
            FlowRouter.go('restaurant.locationTransfer');
            $('#product-barcode').focus();
        }
    }, 500);
});
Template.restaurant_locationTransfer.helpers({
    imeis: function () {
        var locationTransferDetailId = Session.get('locationTransferDetailId');
        if (locationTransferDetailId != null) {
            var sd = Restaurant.Collection.LocationTransferDetails.findOne(locationTransferDetailId);
    ...
    locations: function () {
        return Restaurant.Collection.Locations.find({branchId: Session.get('currentBranch')});
    },
    toLocations: function () {
        return Restaurant.Collection.Locations.find({
            branchId: Session.get('currentBranch'),
    ...
    locationTransferDate: function () {
        var locationTransfer = Restaurant.Collection.LocationTransfers.findOne(FlowRouter.getParam('locationTransferId'));
    ...
    locationTransfer: function () {
        ...
        return Restaurant.Collection.LocationTransfers.findOne(FlowRouter.getParam('locationTransferId'));
    },
    locationTransferDetails: function () {
        var locationTransferDetailItems = [];
        var sD = Restaurant.Collection.LocationTransferDetails.find({locationTransferId: FlowRouter.getParam('locationTransferId')});
        var i = 1;
    ...
    staffs: function () {
        var userStaff = Restaurant.Collection.UserStaffs.findOne({userId: Meteor.user()._id});
        if (userStaff != null) {
    ...
    products: function () {
        return Restaurant.Collection.Products.find({status: "enable"});
        /*.map(function (p) {
         var unit = Restaurant.Collection.Units.findOne(p.unitId).name;
         p.name = p.name + "(" + unit + ")";
         return p;
         });*/
    },
    locationTransfers: function () {
        ...
        if (id != null || id != "") {
            return Restaurant.Collection.LocationTransfers.find({
                _id: {$ne: id},
                branchId: Session.get('currentBranch'),
                status: "Unsaved"
            });
        } else {
            return Restaurant.Collection.LocationTransfers.find({branchId: Session.get('currentBranch'), status: "Unsaved"})
        }
    }
});
Template.restaurant_locationTransfer.events({
    'keyup #input-imei': function (e) {
        if (e.which == 13) {
            ...
            var locationTransferDetailId = Session.get('locationTransferDetailId');
            var locationTransferDetail = Restaurant.Collection.LocationTransferDetails.findOne(locationTransferDetailId);
            var inventoryType = 1;
            var inventory;
            if (inventoryType == 1) {
    ...
    'click .resume': function (e) {
        ...
        var locationTransfer = Restaurant.Collection.LocationTransfers.findOne(locationTransferId);
    ...
    'click .btn-remove': function () {
        Restaurant.Collection.LocationTransferDetails.remove(this._id);
        var ltd = Restaurant.Collection.LocationTransferDetails.find({
            locationTransferId: FlowRouter.getParam('locationTransferId'),
            isPromotion: {$ne: true}
        });
        if (ltd.count() == 0) {
            Restaurant.Collection.LocationTransfers.remove(FlowRouter.getParam('locationTransferId'));
            FlowRouter.go('restaurant.locationTransfer');
            prepareForm();
        }
        /*else {
    ...
    'click .staffInsertAddon': function () {
        alertify.userStaff(fa('plus', 'Add New Staff'), renderTemplate(Template.restaurant_userStaffInsert));
        // .maximize();
    },
    'click .locationInsertAddon': function () {
        alertify.location(fa('plus', 'Add New Location'), renderTemplate(Template.restaurant_locationInsert));
        // .maximize();
    },
...
function locationTransferStock(productId, newQty, branchId, locationTransferId, locationId) {
    var data = {};
    var product = Restaurant.Collection.Products.findOne(productId);
    if (product.productType == "Stock") {
        ...
        if (inventory != null) {
            ...
            debugger;
            var unSavedLocationTransferId = Restaurant.Collection.LocationTransfers.find({
                status: "Unsaved",
                branchId: Session.get('currentBranch'),
            ...
            var otherLocationTransferDetails = Restaurant.Collection.LocationTransferDetails.find({
...
function getValidatedValues(fieldName, val, branchId, locationTransferId) {
    ...
    var product;
    if (fieldName == 'id') {
        product = Restaurant.Collection.Products.findOne(val);
    } else {
        product = Restaurant.Collection.Products.findOne({barcode: val, status: "enable"});
    }
    if (product != null) {
        var defaultQuantity = $('#default-quantity').val() == "" ? 1 : parseInt($('#default-quantity').val());
        if (product.productType == "Stock") {
            var sd = Restaurant.Collection.LocationTransferDetails.findOne({
                productId: product._id,
                branchId: branchId,
    ...
    if (locationTransferId == '') {
        ...
        Meteor.call('insertLocationTransferAndLocationTransferDetail', locationTransferObj, locationTransferDetailObj, function (e, r) {
            ...
            if (e) {
                ...
                $('#product-barcode').focus();
                $('#product-id').select2('val', '');
                FlowRouter.go('restaurant.locationTransfer', {locationTransferId: r});
            }
        });
    } else {
        var locationTransferDetail = Restaurant.Collection.LocationTransferDetails.findOne({
            productId: product._id,
            locationTransferId: locationTransferId
        });
        if (locationTransferDetail == null) {
            var locationTransferDetailObj = {};
            locationTransferDetailObj._id = idGenerator.genWithPrefix(Restaurant.Collection.LocationTransferDetails, locationTransferId, 3);
